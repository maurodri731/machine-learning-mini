        ...
        while True:
            ...
            for x, y in dataset.iterate_once(1):
                ...
                if prediction != actual:
                    update = nn.Constant(actual * x.data)
                    self.w.update(update, 1.0)
                    correct = False
            if correct:
                break
...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        h = nn.ReLU(h)
        o = nn.Linear(h, self.w2)
        o = nn.AddBias(o, self.b2)
        return o
    ...
    def get_loss(self, x, y):
        ...
        "*** YOUR CODE HERE ***"
        predicts = self.run(x)
        return nn.SquareLoss(predicts, y)
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(batch_size):
                ...
                iterate = zip([self.w1, self.b1, self.w2, self.b2], gradients)
                for param, grad in iterate:
                    param.update(grad, -learning_rate)

            full_loss = self.get_loss(nn.Constant(dataset.x), nn.Constant(dataset.y))
            if nn.as_scalar(full_loss) <= loss_threshold:
                break
...
class DigitClassificationModel(object):
    ...
    def get_loss(self, x, y):
        ...
        "*** YOUR CODE HERE ***"
        predicts = self.run(x)
        return nn.SoftmaxLoss(predicts, y)
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        for x in xs[1:]:
            c = nn.Add(nn.Linear(x, self.wx), nn.Linear(h, self.wh))
            c = nn.AddBias(c, self.bh)
            h = nn.ReLU(c)

        o = nn.Linear(h, self.w_output)
        o = nn.AddBias(o, self.b_output)
        return o
    ...
    def train(self, dataset):
        ...
        while True:
            for xs, y in dataset.iterate_once(batch_size):
                ...
                gradients = nn.gradients(loss, [self.wx, self.wh, self.bh, self.w_output, self.b_output])
                iterate = zip([self.wx, self.wh, self.bh, self.w_output, self.b_output], gradients)
                for param, grad in iterate:
                    param.update(grad, -learning_rate)
